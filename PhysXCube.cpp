...
CPhysXCube::CPhysXCube()
{		
	//setName("cube");
	//_actor->setName(this->getName().c_str());	
}	

void CPhysXCube::init(NxScene* scene, osg::Vec3f size, osg::Vec3f position, std::string geometry_type, std::string file_name)
{
	m_scene = scene;
	m_size = size;
	m_position = position;
	...
	mp_geode->addDrawable(new osg::ShapeDrawable(new osg::Box(osg::Vec3(0 , 0, 0), m_size.x(), m_size.y(), m_size.z())));	
	//mp_geode = (osg::Geode*)osgDB::readNodeFile(m_file_name.c_str());
	if (mp_geode) {
		m_BodyDesc.angularDamping	= 0.0f;		
		m_BoxDesc.dimensions = NxVec3(m_size.x(), m_size.z(), m_size.y());		
		m_ActorDesc.shapes.pushBack(&m_BoxDesc);
		m_ActorDesc.body= &m_BodyDesc;
		m_ActorDesc.density= 10.0f;
		m_ActorDesc.globalPose.t  = NxVec3(m_position.x(), m_position.y(), m_position.z());
		
		m_actor = scene->createActor(m_ActorDesc);
	...
	mp_matrix_transform->setMatrix(osg::Matrix::scale(m_size)*osg::Matrix::translate(m_position));

	m_applyShaderColor = false;
	if (m_applyShaderColor) {
		mp_matrix_transform->setUpdateCallback(new MotionCallback(m_actor, mp_matrix_transform->getOrCreateStateSet()));	
	}
	else {
		mp_matrix_transform->setUpdateCallback(new MotionCallback(m_actor));
	}	
}
