...
CPhysXRope::CPhysXRope(NxScene& nxScene,NxVec3& startPosition, int numSegments, float segLength, float segMass,float segCollisionRadius,
...
NxActor*	CPhysXRope::getSegment(int segIndex)	
{
	if(segIndex < numSegs)
...
int			CPhysXRope::getNumSegments()
{
	return numSegs;
}
NxActor*	CPhysXRope::getFirstSegment()
{
...
NxActor*    CPhysXRope::getLastSegment()
{
	if(ropeSegments != NULL)
	{
...
void		CPhysXRope::setUniformVelocity(NxVec3& vel)
{
}

NxJoint*		CPhysXRope::pinFirstSegmentInPlace()
{
	NxSphericalJointDesc sphericalDesc;			
	// some damping on twist and swing springs helps smooth out the rope motion
	//sphericalDesc.flags |= NX_SJF_SWING_SPRING_ENABLED;
...
NxJoint*		CPhysXRope::pinLastSegmentInPlace()
{
	NxSphericalJointDesc sphericalDesc;			
	// some damping on twist and swing springs helps smooth out the rope motion
	//sphericalDesc.flags |= NX_SJF_SWING_SPRING_ENABLED;
...
NxJoint*		CPhysXRope::pinSegmentToPosition(int segIndex, NxVec3& pos)
{
	if(segIndex < numSegs)
	{
	NxSphericalJointDesc sphericalDesc;			
