...
NxVec3 Vec3dToNxVec3(osg::Vec3d vec)
{
	NxVec3 NxVec;
	NxVec.x  = vec.x();
	NxVec.y  = vec.y();
...
bool CEvent_MoveGlobal::Create(CEventDesign* p_design, CEnvironment* p_env)
{	
	m_index_actor = p_env->index_from_name(p_design->m_ref_name, p_env->mv_PhysX_actor);	
	m_p0 = Vec3dToNxVec3(p_design->m_position);
	
	/*CPositionAttitudeTransform *p_PAT = p_env->get_PAT(m_index_PAT);
	if(p_PAT!= NULL) 
		p_PAT->mp_PAT->setPosition(m_p0);	*/

	m_velocity  = Vec3dToNxVec3(p_design->m_velocity);
	m_acceleration  = Vec3dToNxVec3(p_design->m_acceleration);
	m_orientation_axis = Vec3dToNxVec3(p_design->m_orientation_axis);	
...
bool CEvent_MoveGlobal::Update(CEnvironment* p_env)
{	 
	...
	{	*/	
		...
		p_env->mv_PhysX_actor[m_index_actor].mp_nx_actor->moveGlobalPosition(m_position);
