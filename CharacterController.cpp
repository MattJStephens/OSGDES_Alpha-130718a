...
PhysXCController::PhysXCController(NxControllerManager* gManager, NxScene* gScene, 
								   osg::Vec3f origin) : 
_gManager(gManager), 
_gScene(gScene), 
_origin(origin)
...
NxController* PhysXCController::getController()
{
	return _gManager->getController(0);
}

bool PhysXCController::calcDisplacement()
{
	// tmp
	return false;
}

NxExtendedVec3 PhysXCController::getCharacterPos(NxU32 characterIndex)
{
	return _gManager->getController(characterIndex)->getFilteredPosition();
}

osg::Vec3f PhysXCController::getCurrentPos(NxU32 characterIndex)
{
	// this should be changed to allow for retrieving the controller;s position. tmp.
	return _position;
}

NxActor* PhysXCController::getCharacterActor(NxU32 characterIndex)
{
	return _gManager->getController(characterIndex)->getActor();
}

bool PhysXCController::resetCharacterPos(NxU32 index, const NxVec3& pos)
{
...
NxU32 PhysXCController::moveCharacter(NxU32 characterIndex, NxScene* scene, 
...
void PhysXCController::updateControllers()
{
