...
// create sphere
osg::Geode* Cube::init(NxScene* scene, osg::Vec3f size, osg::Vec3f position)
{
	osg::Geode* _geo_cube = (osg::Geode*)osgDB::readNodeFile("I:\\OSG\\cube.osg");
	if (_geo_cube)
	{
		NxBodyDesc BodyDesc;
...
osg::Group* createTower(NxScene* scene, osg::Vec3f origin, int n)
{
	...
	for (int i=0; i<n; i++)
	{
		...
		Cube* cube = new Cube(scene, osg::Vec3f(size, size, size), position );
		group->addChild(cube);		
	}
