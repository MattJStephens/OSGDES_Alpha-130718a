...
//Test SoftBody Implementation - osg + PhysX, osg stuff by Janusz Goldasz
//For the soft body part you need a surface mesh for visualisation + tetrahedral mesh for calculations of the soft body deformations
...
// -----------------------------------------------------------------------
CPhysXSoftBody::CPhysXSoftBody(NxScene *scene, NxSoftBodyDesc &desc, char *tetFileName, ObjMesh *surfaceMesh)
{
	mInitDone = false;
	...
	//_geode = (osg::Geode*) osgDB::readNodeFile("c:\\libs\\openscenegraph-data\\sphereNormalized.obj");
	_geode = new osg::Geode;
	if (_geode)
		addChild(_geode);
	//setMatrix(osg::Matrix::translate(osg::Vec3f(desc.globalPose.t.x, desc.globalPose.t.z, desc.globalPose.t.y)));
	setUpdateCallback(new SoftBodyCallback(scene, this));
}


// -----------------------------------------------------------------------
CPhysXSoftBody::CPhysXSoftBody(NxScene *scene, NxSoftBodyDesc &desc, SoftBodyType type, NxReal wx, NxReal wy, NxReal wz, NxReal h, bool tearLines)
{
	mInitDone = false;
	NxSoftBodyMeshDesc meshDesc;
	switch (type) {
...
// -----------------------------------------------------------------------
void CPhysXSoftBody::init(NxScene *scene, NxSoftBodyDesc &desc, NxSoftBodyMeshDesc &meshDesc, ObjMesh *surfaceMesh)
{
	mScene = scene;
	mScale = 1.0f;
	mOffset.zero();
...
// -----------------------------------------------------------------------
bool CPhysXSoftBody::generateTetMeshDesc(NxSoftBodyMeshDesc &desc, char *filename)
{
...
// -----------------------------------------------------------------------
void CPhysXSoftBody::releaseMeshDescBuffers(const NxSoftBodyMeshDesc& desc)
{
...
// -----------------------------------------------------------------------
bool CPhysXSoftBody::cookMesh(NxSoftBodyMeshDesc& desc)
{
...
// -----------------------------------------------------------------------
void CPhysXSoftBody::allocateReceiveBuffers(int numVertices, int numTetrahedra)
{
...
// -----------------------------------------------------------------------
void CPhysXSoftBody::releaseReceiveBuffers()
{
	NxVec3* vp;
	NxU32* up; 
...
// -----------------------------------------------------------------------
void CPhysXSoftBody::simulateAndDraw(bool shadows, bool tetraMesh, bool tetraWireframe)
{
	tetraMesh= true;
	...
	// osg pieces - JG
	{
		...
		for(NxU32 i = 0; i < numTetrahedra; i++)
		{
			...
			verts->push_back(osg::Vec3f(v[0].x, v[0].z, v[0].y));
			verts->push_back(osg::Vec3f(v[1].x, v[1].z, v[1].y));
			verts->push_back(osg::Vec3f(v[2].x, v[2].z, v[2].y));
			verts->push_back(osg::Vec3f(v[3].x, v[3].z, v[3].y));
		...
		pointsGeom->addPrimitiveSet(new osg::DrawArrays(osg::PrimitiveSet::POINTS,0,verts->size()));
		...
		static int ip=0;
		if (ip!=0)
			_geode->replaceDrawable(_geode->getDrawable(0), pointsGeom);
		else
			_geode->addDrawable(pointsGeom);
		ip++;		
	}
...
// -----------------------------------------------------------------------
void CPhysXSoftBody::generateBoxMeshDesc(NxSoftBodyMeshDesc &desc, NxReal wx, NxReal wy, NxReal wz, NxReal h, bool tearLines)
{
	int numX = (int)(wx / h) + 1;
	int numY = (int)(wy / h) + 1;
	int numZ = (int)(wz / h) + 1;
...
//---------------------------------------------------------------------------------------
int CPhysXSoftBody::childNr(int i0, int i1, NxVec3 &pos) 
{
	int i,j;
...
//---------------------------------------------------------------------------------------
void CPhysXSoftBody::sphereSubdivide(const NxVec3 &p0, const NxVec3 &p1, const NxVec3 &p2, 
...
// -----------------------------------------------------------------------
void CPhysXSoftBody::generateSphereMeshDesc(NxSoftBodyMeshDesc &desc, NxReal wx, NxReal wy, NxReal wz, NxReal h, bool tearLines)
{
	int level = 0;
	NxReal d = 5*h;
...
// soft body demo scene
osg::Group* createDemoSoftBody(NxScene* scene, osg::Vec3f origin, NxArray<CPhysXSoftBody*> &sblist)
{
	osg::Group* group = new osg::Group;

	{
		NxSoftBodyDesc softBodyDesc;
		softBodyDesc.globalPose.t = NxVec3(0.0, 15.0, 0.0);
		...
		CPhysXSoftBody* softbody = new CPhysXSoftBody(scene, softBodyDesc, tetFileName, objMesh);

		group->addChild( softbody );
		sblist.push_back(softbody);
	}
	return group;
}